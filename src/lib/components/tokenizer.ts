...
export class Tokenizer {
	...
	async start () {
...

		{
			...
			this.pdfVersion = m[1]
			if (!this.engine.constants.VERSION_DATA[this.pdfVersion]) {
				this.warnings.push(new PdfError(`Unsupported PDF version: ${this.pdfVersion}`, 'parser:version', { pdfVersion: this.pdfVersion }))
			}
			...
			switch (token.type) {
				...
				case 'ref':
					...
					if (
						...
						this.warnings.push(new PdfError(`Invalid token (object) at offset ${token.start}`, 'token:bad:os', { token }))
						num = -1
						gen = -1
					}
					else {
	...
	async _nextRawToken (): Promise<Token> {
		...
		if (char === '%') {
			...
			if (raw.substring(raw.length - 2) === '\r\n') {
				...
				str = raw
				this.warnings.push(new PdfError(`Unterminated token (comment) at offset ${start}`, 'token:eof:c', { offset: start }))
			}
		...
		if (char === '<') {
			...
			if (raw.substring(raw.length - 1) === '>') {
				...
				hex = raw
				this.warnings.push(new PdfError(`Unterminated token (hexstring) at offset ${start}`, 'token:eof:hex', { offset: start }))
			}
			const data = []
		...
		if (char === '(') {
			...
			if (depth) {
				this.warnings.push(new PdfError(`Unterminated token (string) at offset ${start}`, 'token:eof:s', { offset: start }))
			}
			const end = this.reader.offset
			const str = strParts
				.join('')
		...
		if (this.engine.constants.TOKEN_BYTE_NUMBER.includes(byte)) {
			...
			if (Number.isNaN(num)) {
				num = 0
				this.warnings.push(new PdfError(`Invalid token (real number) "${raw}" at offset ${start}`, 'token:bad:n', { offset: start, type: 'real' }))
			}
		...
		if (this.engine.constants.TOKEN_BYTE_KEYWORD.includes(byte)) {
			...
			switch (keyword) {
				...
				case 'stream': {
					...
					if (!streamEnd) {
						this.warnings.push(new PdfError(`Unterminated token (stream) at offset ${start}`, 'token:eof:stream', { offset: start, type: 'stream' }))
				...
				case 'xref': {
					...
					if (!m) {
						this.warnings.push(new PdfError(`Invalid token (xref head) at offset ${start}`, 'token:bad:xref_head', { offset: start, type: 'xref' }))
						break
					}
					...
					for (let lineIndex = 0; lineIndex < objCount; lineIndex++) {
						...
						if (!m) {
							this.warnings.push(new PdfError(`Invalid token (xref entry) at offset ${start}`, 'token:bad:xref_entry', { offset: start, type: 'xref' }))
							break
						}
				...
				case 'startxref': {
					...
					if (eofStr !== '%%EOF\r\n' && eofStr !== '%%EOF\n') {
						this.warnings.push(new PdfError(`Invalid token (end-of-file marker) at offset ${start}`, 'token:bad:eof', { offset: start, type: 'eof' }))
					}
					const end = this.reader.offset
